...
namespace apim_utils.Supports
{
    /// <summary>
    /// TODO:
    /// We need to make the HttpClient static to avoid connection timeouts in Scalout scenarios.
    /// </summary>
    public static class HttpClientExtensions
    {
          ...
          this Uri baseAddress, string requestPath, object payload, Action<HttpClient> configureClient)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = baseAddress;
                configureClient(client);
                var jsonString = payload is string && string.IsNullOrWhiteSpace(payload + "") ? string.Empty :
        ...
        public async static Task<HttpResponseMessage> PostJsonAsync(this Uri uri, 
            object body, string token, string authHeaderName = "Bearer")
        {
            using (var client = new HttpClient())
            {
                ...
                request.Headers.Authorization = new AuthenticationHeaderValue(authHeaderName, token);
